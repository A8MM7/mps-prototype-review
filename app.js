...
admissionApplication = function(c){
  ...
  const decision = a.status === 'submitted'
    ? `<div class="section-title">Decision</div><div style="display:flex;gap:8px;flex-wrap:wrap">${btn('Decline',`openModal('decline-application',{caseId:'${c.id}'})`,'danger')}${btn('daddy',`openModal('withdraw-application',{caseId:'${c.id}'})`,'secondary')}${btn('Waitlist',`openModal('waitlist-application',{caseId:'${c.id}'})`,'secondary')}${btn('Accept',`openModal('accept-application',{caseId:'${c.id}'})`,'primary')}</div>`
    : '';
