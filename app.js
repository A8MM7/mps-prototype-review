...
function renderAttendance(){return attendanceWorkspace()}
...
function recordAttendanceArrival(childId){
  ...
  if(!prepareAttendanceDailyWrite(childId))return false;
...
function checkInAttendanceRow(childId){
  const status=attendanceCurrentRecord(childId)?.status;
  if(status&&!['not_marked','absent'].includes(status))return;
...
function saveCalendarEvent(){return saveCalendarEntry()}
...
function modalView(m){let n=m.name,d=m.data||{};
  ...
  if(n==='checkout'||n==='temporary-pickup')return attendanceCheckoutModal(d.childId,n==='temporary-pickup');
  if(n==='attendance-correction')return attendanceArrivalCorrectionModal(d.childId);
...
function withdrawApplication(id){let c=db.admissions[id];if(!persistAdmissionsClosure(id,'Withdrawn',val('withdraw_reason'),{type:'withdrawn',title:'Application withdrawn by family',detail:`${val('withdraw_reason')} · ${val('withdraw_note')}`}))return;closeOverlay()}
...
function recordEnquiryContactOutcome(id){
  if(!admissionCanClose(db.admissions[id]))return;
  ...
  if(outcome === 'Not proceeding'){
    ...
    if(!persistAdmissionsClosure(id,'Closed',reason,{type:'non_conversion',title:'Admissions journey closed',detail:`${reason} · ${method}${detailNote}`}))return;
    closeOverlay();
  }
}
...
completeTour = function(id){
  ...
  if(outcome==='Family not proceeding'){
    if(!persistAdmissionsClosure(id,'Withdrawn','Family not proceeding',{type:'tour',title:'Visit completed',detail:details}))return;
  }else addEvent(id,'tour','Visit completed',details);
  closeOverlay();
};
...
recordOverdueDecision=function(id){
  ...
  if(action==='Extend deadline'){
    ...
    if(!persistAdmissionsClosure(id,'Released',reason,{type:'released',title:'Accepted place released',detail:`${reason} · ${actor}`}))return;
  }
  closeOverlay();
};
...
render();
// Owner review — compact rare record-management action and clear Admissions identity.
// Rare record management should stay accessible without competing with normal case work.
// Admissions identity should be explicit: child, parent/guardian and contact details must not rely on inference.
...
const _mpsIdentityAdmissionOverview=admissionOverview;
admissionOverview=function(c){
  const html=_mpsIdentityAdmissionOverview(c);
  const marker='<h3>Admission details</h3>';
  if(!html.includes(marker)) return html;
  const identity=`${kv('Child',esc(c.childName))}${kv('Parent/guardian',esc(c.guardian))}${kv('Phone',esc(c.phone))}`;
  return html.replace(marker,`${marker}${identity}`);
};

function mpsFilterEstablishedDuplicateRecords(query){
...
function mpsSelectEstablishedDuplicateRecord(id){
  const select=byId('est_dup_survivor');
...
modalView=function(m){
  const n=m.name,d=m.data||{};
  if(n!=='mark-established-duplicate') return _mpsSearchableDuplicateModalView(m);

  const c=db.admissions[d.caseId];
  if(!c) return _mpsSearchableDuplicateModalView(m);
  const options=mpsEstablishedDuplicateOptions(c.id);
...
render();
// BQ-098 — retain pre-enrolment closure context without adding a lifecycle stage.
function admissionClosureStage(c){
  if(c.closed?.stage) return c.closed.stage;
  if(c.application?.status==='declined') return 'Application';
...
function admissionClosureLabel(c){
  ...
  if(stage==='Application') return type==='Declined'?'Application declined':'Family withdrew during Application';
  if(['Accepted','Admission Fee'].includes(stage)) return `${type==='Released'?'Place released':'Family withdrew'} after acceptance · ${stage}`;
  return `Closed ${stage==='Visit'?'after':'at'} ${stage}`;
}
...
admissionMatchesFilter=function(c,filter){
  ...
  if(filter==='Closed') return admissionIsApplicationClosed(c);
  if(c.closed) return false;
  return _bq098Matches(c,filter);
};
function admissionIdentityMatches(c,q){
...
filterAdmissions=function(q){
  admissionsSearchQuery=q;
  const list=byId('admissionsCaseItems');
...
setAdmissionCase=function(id){
  ...
  admissionsSearchQuery='';_bq098Select(id);
};
...
caseListItem=function(c){
  if(!admissionIsClosedCase(c))return _bq098CaseItem(c);
  const outcome=admissionSecondaryOutcome(c);
  return `<div class="case-item ${ui().admissionsCase===c.id?'active':''}" data-stage="Closed" onclick="setAdmissionCase('${c.id}')"><div class="top"><strong>${esc(c.childName)}</strong><span>${badge(outcome||'Closed','grey')}</span></div><div class="meta">${esc(c.guardian)} · ${esc(c.service)}<div class="closure-context">${esc(admissionClosureLabel(c))} · ${esc(c.closed.reason||'Reason not recorded')}</div></div></div>`;
};
const _bq098Overview=admissionOverview;
admissionOverview=function(c){
  const html=_bq098Overview(c);
  if(!admissionCanClose(c))return html;
  const action=`<div class="admission-close-action">${btn('Close admissions case',`openModal('close-admissions-case',{caseId:'${c.id}'})`,'secondary','sm')}</div>`;
  const end=html.lastIndexOf('</div>');
  return html.slice(0,end)+action+html.slice(end);
};
function admissionClosureOptions(c){
  const stage=admissionDisplayStage(c);
  if(stage==='Application')return ['Declined','Family withdrew'];
  if(['Accepted','Admission Fee'].includes(stage))return ['Place released','Family withdrew after acceptance'];
  return ['Closed'];
}
// One retained closure write for generic and contextual pre-enrolment actions.
// Callers retain their forms and supply their existing contextual history wording.
function persistAdmissionsClosure(id,type,reason,event){
  const c=db.admissions[id];
  if(!admissionCanClose(c)||!String(reason||'').trim())return false;
  const stage=admissionDisplayStage(c),at=new Date().toISOString(),by=mpsAdmissionActor();
  c.closed={type,stage,reason:String(reason).trim(),at,by};
  if(type==='Released')Object.assign(c.closed,{releasedAt:at,releasedBy:by});
  c.followUp=null;
  const history=event||{type:'case_closed',title:admissionClosureLabel(c),detail:`${c.closed.reason} · ${by}`};
  c.events.push({...ev(history.type,history.title,history.detail),at});
  ui().admissionsStageFilter=stage==='Application'?'Closed':'Closed cases';
  ui().admissionsCase=id;ui().admissionsTab='overview';admissionsSearchQuery='';
  save();
  return true;
}
function closeAdmissionsCase(id){
  const c=db.admissions[id];if(!admissionCanClose(c))return;
  const reason=val('case_close_reason').trim(),outcome=val('case_close_outcome')||'Closed';
  if(!reason){alert('Enter the reason for closing this admissions case.');return}
  if(!admissionClosureOptions(c).includes(outcome))return;
  const stage=admissionDisplayStage(c);
  const type={'Place released':'Released','Family withdrew':'Withdrawn','Family withdrew after acceptance':'Withdrawn'}[outcome]||outcome;
  if(!persistAdmissionsClosure(id,type,reason))return;
  if(stage==='Application'&&type==='Declined')c.application.status='declined';
  closeOverlay();
}
const _bq098Modal=modalView;
modalView=function(m){
  if(m.name!=='close-admissions-case')return _bq098Modal(m);
  const c=db.admissions[m.data?.caseId];
  if(!admissionCanClose(c))return modal('Case closure unavailable','',notice('Only authorised Admissions staff can close an active pre-enrolment case.','warn'),btn('Close','closeOverlay()','secondary'));
  const options=admissionClosureOptions(c);
  return modal('Close admissions case',c.childName,`${kv('Current stage',esc(admissionDisplayStage(c)))}${options.length>1?selectField('Outcome',options,options[0],'case_close_outcome'):''}${textArea('Reason (required)','','case_close_reason')}${notice('The case and its history will be retained in Closed cases.','info')}`,`${btn('Cancel','closeOverlay()','secondary')}${btn('Close case',`closeAdmissionsCase('${c.id}')`,'danger')}`);
};
// Historical cases offer evidence, not controls that could resume their journey.
const _bq098Tab=renderAdmissionTab;
renderAdmissionTab=function(c){
  if(!admissionIsClosedCase(c))return _bq098Tab(c);
  const summary=`<div class="card closure-summary"><h3>${esc(admissionClosureLabel(c))}</h3>${kv('Reason',esc(c.closed.reason||'Reason not recorded'))}${c.closed.by?kv('Closed by',esc(c.closed.by)):''}${c.closed.at?kv('Closed on',fmtDate(c.closed.at.slice(0,10))):''}</div>`;
  if(ui().admissionsTab==='timeline')return summary+admissionTimeline(c);
  const html=_bq098Tab(c).replace(/<button\b[^>]*>[\s\S]*?<\/button>/g,'');
  return summary+html;
};
// Closed cases cannot be progressed through stale staff or parent controls.
function admissionUnlessClosed(fn){return function(id,...args){if(db.admissions[id]?.closed)return;return fn(id,...args)}}
qualifyCase=admissionUnlessClosed(qualifyCase);
scheduleTour=admissionUnlessClosed(scheduleTour);
sendApplication=admissionUnlessClosed(sendApplication);
submitApplication=admissionUnlessClosed(submitApplication);
acceptApplication=admissionUnlessClosed(acceptApplication);
waitlistApplication=admissionUnlessClosed(waitlistApplication);
returnWaitlistedApplication=admissionUnlessClosed(returnWaitlistedApplication);
issueAdmissionFee=admissionUnlessClosed(issueAdmissionFee);
createEnrolment=admissionUnlessClosed(createEnrolment);
render();

// Preserve the point reached in the existing journey graphic for historical cases.
const _bq098Journey=journey;
journey=function(c){
  if(!admissionIsClosedCase(c))return _bq098Journey(c);
  const stage=admissionClosureStage(c),copy={...c,closed:null};
  if(stage==='Application'&&copy.application?.status==='declined')copy.application={...copy.application,status:'submitted'};
  return _bq098Journey(copy);
};
render();
...
function attendanceRoster(){
  return Object.values(db.people.children).filter(c=>currentOperationalChild(c.id)&&classInScope(childClass(c))).map(c=>({
    ...(attendanceCurrentRecord(c.id)||{}),id:c.id,name:c.legalName||c.preferred,
    className:childClass(c),status:attendanceCurrentRecord(c.id)?.status||'not_marked'
  }));
}
...
function ensureProfilePeople(){
  ...
  changed=ensureAttendanceReviewSamples()||changed;
  changed=ensureAttendanceRecordDates()||changed;
  if(changed)save();
}
...
modalView=function(m){
  ...
  if(['checkout','temporary-pickup'].includes(m?.name)&&canViewPickup(m.data?.childId))html=html.replace('<div class="modal-body">',`<div class="modal-body">${profilePickupIdentity(m.data.childId,activeTemporaryPickup(m.data.childId)?'':null)}`);
  return html;
};
...
// BQ-102: one Child-owned instruction; Attendance consumes it at handover.
function temporaryPickupInstructions(childId){return db.people?.children?.[childId]?rawFamilyRecord(childId).temporaryPickups||[]:[]}
function activeTemporaryPickup(childId){
  return temporaryPickupInstructions(childId).find(p=>p.date===TODAY)||null;
}
...
function ensureAttendanceReviewSamples(){
  if(db.attendanceReviewSamples)return false;
  // Move the exact old demonstration instruction, retaining unknown attribution.
  const old=db.attendance.dilan?.temporaryPickup;
  if(old){
    const f=editableFamily('dilan');f.temporaryPickups=f.temporaryPickups||[];
    if(!f.temporaryPickups.some(p=>p.id==='dilan-temporary-sample'))f.temporaryPickups.push({
      id:'dilan-temporary-sample',childId:'dilan',name:old.name,date:TODAY,
      from:'16:00',until:'18:00',requestedBy:'Registered guardian',source:'WhatsApp',
      requestContext:old.source,reference:old.reference,legacyValidity:old.valid,
      recordedAt:null,recordedActor:null,history:[],origin:'Existing demonstration instruction'
    });
    delete db.attendance.dilan.temporaryPickup;
  }
  const r=db.attendance.kavindu;
  if(r?.status==='absent'&&!r.absence){
    r.absence={childId:'kavindu',date:TODAY,confirmation:'Staff confirmed',note:'',
      recordedAt:TODAY+'T08:30:00+05:30',actor:{staffId:'anjali',name:'Anjali Fernando',roles:['Head Teacher']},origin:'Owner-approved demonstration reconciliation'};
  }
  db.attendanceReviewSamples=true;return true;
}
...
function temporaryPickupInstructionModal(childId,id){
  ...
  const p=id?temporaryPickupInstructions(childId).find(x=>x.id===id):null;
  if(id&&!p)return modal('Temporary pickup','',notice('Instruction not found.','warn'),btn('Close','closeOverlay()','secondary'));
  return modal(p?'Edit temporary pickup':'Add temporary pickup','Record the guardian’s instruction. Staff must still verify the collector before handover.',
    `${kv('Child',esc(profileChildName(childId)))}${field('Temporary collector name',p?.name||'','text',false,'tp_name')}${field('Valid date',p?.date||TODAY,'date',false,'tp_date')}<div class="grid"><div class="span-6">${field('From (optional)',p?.from||'','time',false,'tp_from')}</div><div class="span-6">${field('Until (optional)',p?.until||'','time',false,'tp_until')}</div></div>${field('Requested / authorised by',p?.requestedBy||'','text',false,'tp_requester')}${selectField('Request source',['Select…','WhatsApp','Phone','In person'],p?.source||'Select…','tp_source')}${textArea('Reference / verification information',p?.reference||'','tp_reference')}`,
    btn('Cancel','closeOverlay()','secondary')+btn('Save instruction',`saveTemporaryPickupInstruction('${childId}','${id||''}')`,'primary'));
}
function saveTemporaryPickupInstruction(childId,id){
  ...
  if(!name||!/^\d{4}-\d{2}-\d{2}$/.test(date)||!requestedBy||!['WhatsApp','Phone','In person'].includes(source)||!reference){alert('Record the collector, valid date, requester, request source and verification information.');return}
  if((!!from!==!!until)||(from&&from>=until)){alert('Record both times with the end after the start, or leave the time window empty.');return}
  if(temporaryPickupInstructions(childId).some(p=>p.id!==id&&p.date===date)){alert('An instruction is already recorded for this date. Edit that instruction instead.');return}
  const f=editableFamily(childId);f.temporaryPickups=f.temporaryPickups||[];
  const at=new Date().toISOString(),actor=staffActor();
  const p={id:id||'temporary_'+crypto.randomUUID(),childId,name,date,from,until,requestedBy,source,reference,
    recordedAt:existing?existing.recordedAt:at,recordedActor:existing?existing.recordedActor:actor,
    history:[...(existing?.history||[]),{at,actor,action:existing?'Instruction updated':'Instruction recorded',before:existing?{name:existing.name,date:existing.date,from:existing.from,until:existing.until,requestedBy:existing.requestedBy,source:existing.source,reference:existing.reference}:null}]};
  if(existing)f.temporaryPickups[f.temporaryPickups.findIndex(x=>x.id===id)]=p;else f.temporaryPickups.push(p);
  closeOverlay();
}
// BQ-102: two operational views of the same physical-presence record.
function setAttendanceTab(tab){if(!['arrivals','pickup'].includes(tab))return;ui().attendanceTab=tab;save();render()}
function attendanceCanRecord(childId){return allowed('attendance')&&currentOperationalChild(childId)&&classInScope(childClass(db.people.children[childId]))}
function attendanceCanCheckout(childId){return allowed('attendance')&&canViewPickup(childId)&&classInScope(childClass(db.people.children[childId]))&&db.attendance[childId]?.status==='present'}
function attendanceAbsenceModal(childId){
  if(!attendanceCanRecord(childId)||attendanceCurrentRecord(childId)?.status&&attendanceCurrentRecord(childId).status!=='not_marked')return modal('Mark absent','',notice('Only a child with no recorded outcome can be marked absent here.','warn'),btn('Close','closeOverlay()','secondary'));
  return modal('Mark absent','Record a confirmed absence, not a missing check-in.',kv('Child',esc(profileChildName(childId)))+selectField('How was the absence confirmed?',['Select…','Parent/guardian informed us','Staff confirmed'],'Select…','absence_source')+textArea('Optional note','','absence_note'),btn('Cancel','closeOverlay()','secondary')+btn('Mark absent',`recordAttendanceAbsence('${childId}')`,'primary'));
}
function recordAttendanceAbsence(childId){
  if(!attendanceCanRecord(childId))return;
  const current=attendanceCurrentRecord(childId);if(current?.status&&current.status!=='not_marked')return;
  const confirmation=val('absence_source');if(!['Parent/guardian informed us','Staff confirmed'].includes(confirmation)){alert('Choose how the absence was confirmed.');return}
  if(!prepareAttendanceDailyWrite(childId))return;
  const previous=db.attendance[childId],child=db.people.children[childId];
  db.attendance[childId]={...(previous||{}),id:childId,date:TODAY,name:profileChildName(childId),className:childClass(child),status:'absent',checkIn:null,checkOut:null,collector:null,corrections:previous?.corrections||[],
    absence:{childId,date:TODAY,confirmation,note:val('absence_note').trim(),recordedAt:new Date().toISOString(),actor:staffActor()}};
  closeOverlay();
}

function attendanceWorkspace(){
  const rows=attendanceRoster(),pickup=ui().attendanceTab==='pickup';
  const visible=pickup?rows.filter(r=>['present','checked_out'].includes(r.status)).sort((a,b)=>(a.status==='checked_out')-(b.status==='checked_out')):rows;
  const controls=pickup?'':btn('Rapid arrival',"openModal('rapid-arrival')",'primary');
  return shell(`${pageHead('Physical presence','Attendance & pickup',pickup?'Verify the collector before handover. Children stay checked in until checkout is recorded.':'Check children in when they arrive. Record absence only when it is confirmed.',controls)}
    <div class="tabs attendance-tabs" role="tablist" aria-label="Attendance workspace">${[['arrivals','Arrivals'],['pickup','Pickup']].map(([id,label])=>`<button role="tab" aria-selected="${(pickup?'pickup':'arrivals')===id}" class="tab ${(pickup?'pickup':'arrivals')===id?'active':''}" onclick="setAttendanceTab('${id}')">${label}</button>`).join('')}</div>
    <div class="attendance-summary">${rows.filter(r=>r.status==='present').length} on site · ${rows.filter(r=>r.status==='not_marked').length} not checked in · ${rows.filter(r=>r.status==='absent').length} absent · ${rows.filter(r=>r.status==='checked_out').length} checked out</div>
    <div class="table-wrap"><table class="table"><thead><tr><th class="roster-ordinal">#</th><th>Child</th><th>Class</th><th>Status</th><th>Arrival</th>${pickup?'<th>Pickup</th>':''}<th></th></tr></thead><tbody>${visible.map((r,index)=>{
      const instruction=activeTemporaryPickup(r.id);
      const context=r.status==='checked_out'?`${esc(r.collector||'Not recorded')} · ${esc(r.checkOut||'Not recorded')}`:instruction?`<span class="attendance-temporary-cue">Temporary pickup · verify at handover</span>`:'Authorised collector · verify at handover';
      const actions=pickup?(r.status==='present'?btn('Checkout',`openModal('checkout',{childId:'${r.id}'})`,'primary','sm')+(canManageTemporaryPickup(r.id)?btn(instruction?'Edit temporary pickup':'Add temporary pickup',`openModal('temporary-pickup-instruction',{childId:'${r.id}'${instruction?`,id:'${instruction.id}'`:''}})`,'secondary','sm'):''):''):r.status==='not_marked'?btn('Check in',`checkInAttendanceRow('${r.id}')`,'primary','sm')+btn('Mark absent',`openModal('attendance-absence',{childId:'${r.id}'})`,'secondary','sm'):r.status==='absent'?btn('Check in',`checkInAttendanceRow('${r.id}')`,'primary','sm'):attendanceCanCorrectArrival(r.id)?btn('Edit',`openModal('attendance-correction',{childId:'${r.id}'})`,'secondary','sm'):'';
      return `<tr data-child-id="${esc(r.id)}"><td class="roster-ordinal">${index+1}</td><td><div class="name">${attendanceChildIdentity(r.id,r.name)}</div></td><td>${esc(r.className)}</td><td><div>${badge(attendanceStatusLabel(r.status),r.status==='present'?'green':r.status==='absent'?'grey':'blue')}</div></td><td>${esc(r.checkIn||'—')}</td>${pickup?`<td><div>${context}</div></td>`:''}<td><div class="attendance-row-actions">${actions}</div></td></tr>`;
    }).join('')}</tbody></table>${!visible.length?'<div class="empty">No children on site or completed checkouts today.</div>':''}</div>
    ${attendanceTodaysCorrections().length?`<div class="attendance-corrections-summary">${btn(`View corrections (${attendanceTodaysCorrections().length})`,"openModal('attendance-corrections')",'secondary')}</div>`:''}`);
}
function attendanceCheckoutModal(childId,temporaryOnly=false){
  const r=db.attendance[childId];
  if(!attendanceCanCheckout(childId))return modal('Checkout child','',notice('This child is not currently available for checkout in your scope.','warn'),btn('Close','closeOverlay()','secondary'));
  const p=activeTemporaryPickup(childId);
  if(p)return modal('Temporary pickup · '+profileChildName(childId),'Human verification remains the safety decision.',
    kv('Instruction',esc(p.name+' · '+temporaryPickupValidity(p)))+kv('Requested by',esc(p.requestContext||[p.requestedBy,p.source].join(' · ')))+kv('Reference',esc(p.reference))+selectField('Physical identity check',['Reference photo matched by staff','Government photo ID checked','Live visual guardian confirmation'],'Reference photo matched by staff','temp_verify')+notice('If staff are not satisfied, do not release the child.','bad'),btn('Do not release','closeOverlay()','danger')+btn('Escalate uncollected',`escalateUncollected('${childId}')`,'secondary')+btn('Safe handover & checkout',`checkout('${childId}',true)`,'primary'));
  if(temporaryOnly)return modal('Temporary pickup','',notice('No temporary pickup instruction is recorded for today.','warn'),btn('Close','closeOverlay()','secondary'));
  return modal('Checkout child','Record only after safe handover.',profileCollectorField(childId)+kv('Verification','Routine authorised collector · manual human check'),btn('Cancel','closeOverlay()','secondary')+btn('Record checkout',`checkout('${childId}')`,'primary'));
}

// Current and historical daily records have one owner: Attendance. The Child
// profile reads them; it never stores a copied Attendance history.
...
function prepareAttendanceDailyWrite(childId){
  ...
  if(!previous?.date||previous.date===TODAY||previous.status==='present')return true;
  if(previous.date>TODAY)return false;
  db.attendanceHistory=db.attendanceHistory||{};
  const day=db.attendanceHistory[previous.date]||(db.attendanceHistory[previous.date]={});
  if(day[childId]){alert('An attendance record already exists for this date. Review the retained history before recording another event.');return false}
  day[childId]=previous;delete db.attendance[childId];return true;
}
function attendanceCanViewHistory(childId){return allowed('attendance')&&canViewChild(childId)&&classInScope(childClass(db.people.children[childId]))}
function attendanceRecordsForChild(childId){
  const days=Object.entries(db.attendanceHistory||{}).flatMap(([date,records])=>records[childId]?[{date,record:records[childId]}]:[]);
  const current=db.attendance[childId];if(current)days.push({date:current.date||TODAY,record:current});
  return days.sort((a,b)=>b.date.localeCompare(a.date));
}
function attendanceRecordedTime(at){
  if(!at)return 'Not recorded';
  if(!String(at).includes('T'))return String(at);
  const date=new Date(at);if(Number.isNaN(date.getTime()))return String(at);
  return date.toLocaleString('en-GB',{timeZone:db.organization?.timezone||'Asia/Colombo',day:'numeric',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'});
}
function attendanceFact(label,value){return `<div class="attendance-fact"><dt>${esc(label)}</dt><dd>${esc(value??'Not recorded')}</dd></div>`}
function attendanceCorrectionMarkup(correction){
  return `<dl class="attendance-facts">${attendanceFact('Original',attendanceCorrectionValue(correction.from))}${attendanceFact('Corrected',attendanceCorrectionValue(correction.to))}${attendanceFact('Reason',correction.reason)}${attendanceFact('Changed by',correction.actor?.name||correction.by||'Not recorded')}${attendanceFact('Recorded at',attendanceRecordedTime(correction.at))}</dl>`;
}
function attendanceTodaysCorrections(){
  return Object.keys(db.people?.children||{}).filter(attendanceCanViewHistory).flatMap(childId=>
    attendanceRecordsForChild(childId).flatMap(({date,record})=>(record.corrections||[]).filter(c=>(c.date||date)===TODAY).map(c=>({childId,recordDate:date,record,correction:c})))
  );
}
function attendanceCorrectionValue(value){return value&&typeof value==='object'?[value.confirmation,value.note||'No note'].join(' · '):value}
function attendanceCorrectionsModal(){
  const entries=attendanceTodaysCorrections();
  return modal('Today’s corrections','',`<div class="attendance-correction-list">${entries.map(({childId,record,correction:c})=>`<article><header><strong>${esc(record.name||profileChildName(childId))}</strong><span>${c.field==='absence'?'Absence details':'Arrival'}</span></header><p class="attendance-correction-values">${esc(attendanceCorrectionValue(c.from))} → ${esc(attendanceCorrectionValue(c.to))}</p><p>${esc(c.reason)}</p><footer>Changed by ${esc(c.actor?.name||c.by||'Not recorded')} · ${esc(attendanceRecordedTime(c.at))}</footer></article>`).join('')||'<p>No attendance corrections recorded today in your scope.</p>'}</div>`,btn('Close','closeOverlay()','secondary'));
}
function attendanceCanCorrectArrival(childId){
  const r=db.attendance[childId];return attendanceCanViewHistory(childId)&&r?.status==='present'&&!!r.checkIn;
}
function attendanceArrivalCorrectionModal(childId){
  if(!attendanceCanCorrectArrival(childId))return modal('Edit arrival','',notice('This arrival is not available for correction in your scope.','warn'),btn('Close','closeOverlay()','secondary'));
  const r=db.attendance[childId];
  return modal('Edit arrival','The original time and correction reason are retained.',
    `<dl class="attendance-facts">${attendanceFact('Child',profileChildName(childId))}${attendanceFact('Recorded arrival',r.checkIn)}</dl><input type="hidden" id="attendance_original" value="${esc(r.checkIn)}">${field('Correct arrival time',r.checkIn,'time',false,'attendance_corrected')}${textArea('Reason for correction','','attendance_reason')}`,
    btn('Cancel','closeOverlay()','secondary')+btn('Save correction',`correctAttendanceArrival('${childId}')`,'primary'));
}
function correctAttendanceArrival(childId){
  ...
  if(!/^([01]\d|2[0-3]):[0-5]\d$/.test(to)||!reason){alert('Enter the correct arrival time and a reason.');return}
  if(val('attendance_original')!==r.checkIn){alert('The arrival has changed since this form was opened. Close and reopen it to review the current time.');return}
  if(to===r.checkIn){alert('The arrival time is unchanged.');return}
  r.date=r.date||TODAY;
  r.originalCheckIn=r.originalCheckIn||r.corrections?.[0]?.from||r.checkIn;
  r.corrections=r.corrections||[];
  r.corrections.push({from:r.checkIn,to,reason,by:currentPersona().name,byId:currentPersona().id,actor:staffActor(),at:new Date().toISOString(),date:TODAY,recordDate:r.date||TODAY,field:'checkIn'});
  r.checkIn=to;closeOverlay();
}
function attendanceHistoryModal(childId){
  if(!attendanceCanViewHistory(childId))return modal('Attendance history','',notice('Attendance history is outside your current scope.','warn'),btn('Close','closeOverlay()','secondary'));
  const days=attendanceRecordsForChild(childId);
  return modal('Attendance history',profileChildName(childId),`<div class="attendance-history">${days.map(({date,record:r})=>`<article><h3>${esc(fmtDate(date))}</h3>${badge(attendanceStatusLabel(r.status),r.status==='present'?'green':'grey')}<dl class="attendance-facts">${attendanceFact('Arrival',r.checkIn||'Not recorded')}${r.checkIn?attendanceFact('Recorded by',r.arrivalActor?.name||'Not recorded')+attendanceFact('Recorded at',attendanceRecordedTime(r.arrivalRecordedAt)):''}${r.originalCheckIn||r.corrections?.some(c=>c.field!=='absence')?attendanceFact('Original arrival',r.originalCheckIn||r.corrections.find(c=>c.field!=='absence').from):''}${r.absence?attendanceFact('Absence confirmation',r.absence.confirmation)+attendanceFact('Recorded by',r.absence.actor?.name||'Not recorded')+attendanceFact('Recorded at',attendanceRecordedTime(r.absence.recordedAt))+(r.absence.note?attendanceFact('Absence note',r.absence.note):''):''}${attendanceFact('Checkout',r.checkOut||'Not recorded')}${r.checkOut?attendanceFact('Collector',r.collector||'Not recorded')+attendanceFact('Recorded by',r.checkoutActor?.name||'Not recorded')+attendanceFact('Recorded at',attendanceRecordedTime(r.checkoutRecordedAt)):''}${r.pickupVerification?attendanceFact('Verification',r.pickupVerification.method):''}</dl>${(r.corrections||[]).map(c=>`<section class="attendance-history-correction"><h4>${c.field==='absence'?'Absence details correction':'Arrival correction'}</h4>${attendanceCorrectionMarkup(c)}</section>`).join('')}</article>`).join('')||'<p>No attendance records recorded for this child.</p>'}</div>`,btn('Close','closeOverlay()','secondary'));
}

function ensureAttendanceRecordDates(){
  if(db.attendanceDailyDates)return false;
  // Legacy Attendance contains the prototype operational day's samples, not
  // undated historical records. Date them once without creating any event.
  Object.values(db.attendance||{}).forEach(r=>{r.date=r.date||TODAY});
  db.attendanceDailyDates=true;return true;
}
